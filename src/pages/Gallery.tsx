...
import tapperss from "../assets/images/tapperss.png";
...
const galleryItems: GalleryItem[] = [
  ...
  { id: 12, img: tapperss, category: "tapers", label: "Taper" },
];
...
const faqs = [
  {
    ...
    a: "Simply message us on WhatsApp or click the Book Now button.",
  },
  {
    q: "Do I need to pay before my appointment?",
    a: "No, payment is made at the time of service.",
  },
  {
    q: "Do you offer home service visits?",
    a: "Yes! We offer home visits on the Mainland and Island.",
  },
  {
    q: "How much does a haircut cost?",
    a: "Prices start from ₦3,500. Check our Services page for full pricing.",
  },
  {
    q: "Can I bring a reference photo?",
    a: "Absolutely! Reference photos help our barbers deliver exactly what you want.",
  },
  {
    q: "Where is FadeHaus located?",
    a: "We are located at 123 Adeola Odeku St, VI, Lagos.",
  },
  {
  ...
  return (
    <div className="gal-faq-item" onClick={() => setOpen(!open)}>
      <div className="gal-faq-item__top">
        <span>- {q}</span>
        <span className={`gal-faq-item__arrow ${open ? "open" : ""}`}>
          &#x2304;
        </span>
      </div>
  ...
  return (
    <div className="lightbox-overlay" onClick={onClose}>
      <div className="lightbox-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <img src={current.img} alt={current.label} className="lightbox-img" />
        <button className="lightbox-arrow lightbox-arrow--next" onClick={onNext}>&#8250;</button>
        <p className="lightbox-label">{current.label}</p>
        <p className="lightbox-counter">{currentIndex + 1} / {items.length}</p>
      </div>
    </div>
  );
};
...
const Gallery = () => {
  ...
  const filtered =
    activeTab === "ALL"
      ? galleryItems
      : galleryItems.filter(
          (item) => item.category === activeTab.toLowerCase().replace(" ", ""),
        );

  const openLightbox = (index: number) => setLightboxIndex(index);
  const closeLightbox = () => setLightboxIndex(null);
  const prevImage = () => setLightboxIndex((prev) => (prev !== null ? (prev - 1 + filtered.length) % filtered.length : 0));
  const nextImage = () => setLightboxIndex((prev) => (prev !== null ? (prev + 1) % filtered.length : 0));

  return (
    <div className="gallery-page">
      <div className="gal-header">
        <p className="gal-header__label">💈 GALLERY 💈</p>
        <h1 className="gal-header__title">
          ...
          <div className="gal-cta__phone">
            <img
              ...
              style={{
                width: "16px",
                filter:
                  "invert(75%) sepia(50%) saturate(400%) hue-rotate(5deg)",
              }}
              alt="phone"
            />
